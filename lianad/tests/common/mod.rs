...
pub fn wait_for_while_condition_holds<S, C>(success: S, condition: C, timeout: Duration)
where
    S: Fn() -> bool,
    C: Fn() -> bool,
{
    ...
    loop {
        if start.elapsed() > timeout {
            assert!(false, "Timeout waiting for success");
        }

        if !condition() {
            assert!(false, "Condition no longer holds");
        }
...
pub fn wait_for<S>(success: S, timeout: Duration)
where
    S: Fn() -> bool,
{
    wait_for_while_condition_holds(success, || true, timeout)
}
