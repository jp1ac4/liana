...
pub trait DatabaseConnection {
    ...
    /// Get the timestamp at which the last poll completed, if any.
    fn last_poll_timestamp(&mut self) -> Option<u64>;

    /// Set the timestamp at which the last poll completed.
...
impl DatabaseConnection for SqliteConn {
    ...
    fn set_last_poll(&mut self, timestamp: u64) {
        // Sqlite uses i32 for integers so we need to truncate to u32.
