...
impl BitcoinInterface for electrum::Electrum {
    fn rollback_wallet_tip(&mut self, new_tip: &BlockChainTip) {
        self.rollback_wallet_tip(new_tip)
    }
    ...
    fn start_rescan(
        ...
        Err("rescan is not required for electrum".to_string())
    }

    fn rescan_progress(&self) -> Option<f64> {
        // We should never be rescanning.
        None
    }

    fn block_before_date(&self, _timestamp: u32) -> Option<BlockChainTip> {
        unimplemented!("rescan is not required for electrum")
    }
